...
def load_data(fpath1, maxlen1, maxlen2):
    ...
    with open(fpath1, 'r') as f1:
        for sent1, sent2 in zip(f1, f1):
            if len(sent1.strip().replace(' ',''))+1 > maxlen1: continue
            if len(sent2.strip().replace(' ',''))+1 > maxlen2: continue
            sents1.append(sent1.strip().replace('  ',' '))
            sents2.append(sent2.strip().replace('  ',' '))
    return sents1, sents2
...
def get_batch(fpath1, maxlen1, maxlen2, vocab_fpath, batch_size, shuffle=False):
    ...
    num_samples
    '''
    sents1, sents2 = load_data(fpath1, maxlen1, maxlen2)
